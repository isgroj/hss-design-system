...
| Token | Light | Dark | Usage |
|-------|-------|------|-------|
| `--header-bg` | var(--bg-surface) | #1E293B | Header background |
| `--header-nav-active` | var(--text-brand) | #FFFFFF | Active nav link |
| `--header-nav-inactive` | var(--text-secondary) | #475569 | Inactive nav links |
| `--header-icon` | currentColor | #94A3B8 | Bell icon, hamburger |
| `--header-avatar` | var(--action-primary) | #64748B | Avatar circle |
| `--footer-bg` | var(--bg-surface) | #1E293B | Footer background |
| `--footer-border` | var(--border-default) | #334155 | Footer border |
| `--footer-icon-fill` | #0074BC | white | Pulse Grid icon |
...
- Background: `--header-bg` (light: bg-surface, dark: #1E293B)
- Left: HSS logo 32x32 (radius-sm) + nav links (14px, gap 24px)
  - Active: `--header-nav-active` (light: text-brand, dark: white), font-weight 600
  - Inactive: `--header-nav-inactive` (light: text-secondary, dark: #475569)
- Right: bell icon 20x20 + avatar 32x32 circle (`--header-avatar`) + username 14px
- Layout: flexbox, space-between, center-aligned

### App Header — Mobile (375px)
- Height: 52px, padding: 0 16px
- Left: hamburger 24x24 + HSS logo 28x28, gap 12px
- Right: bell icon 20x20 + avatar 28x28, gap 12px
- No username text on mobile

### App Footer — Desktop (1440px)
- Height: 64px, padding: 0 24px
- Background: `--footer-bg`, border-top: 1px `--footer-border`
- Left: Pulse Grid icon 28x28 + "POWERED BY" (13.5px, medium, `--footer-powered-by`) + "HSS Data" (20px, bold, `--footer-brand-text`), gap 10px
- Center: "© 2026 HSS" (12px, `--text-tertiary`)
- Right: "Support & Feedback" pill button — padding 8px 20px, radius 16px, border 1.5px `--footer-btn-border`, font-weight 600, 14px, color `--footer-btn-text`
